...
const DUPLICATE_GSTIN_MESSAGE =
  "This GSTIN is already registered to another supplier or supply location.";

function mapSupplierRow(row) {
  if (!row) return null;
  const locations = (row.locations ?? []).map(mapSupplierLocationRow);
  return {
    id: row.id,
    name: row.name,
...
function mapSupplierLocationRow(row) {
  return {
    id: row.id,
...
async function assertGstinUnique(gstin, opts = {}) {
  ...
  const excludeSupplierId = opts.excludeSupplierId != null ? Number(opts.excludeSupplierId) : null;
  const excludeLocationIds = new Set(
    (opts.excludeLocationIds ?? []).map((id) => Number(id)).filter((id) => Number.isFinite(id) && id > 0),
  );
  const forLocation = Boolean(opts.forLocation);

  const supplierHit = await prisma.supplier.findFirst({
    where: {
      gst: normalized,
  ...
  if (forLocation && excludeSupplierId) {
    const parent = await prisma.supplier.findUnique({
  ...
  const locationHit = await prisma.supplierLocation.findFirst({
    where: {
      gst: normalized,
      ...(excludeLocationIds.size ? { NOT: { id: { in: [...excludeLocationIds] } } } : {}),
    },
    select: { id: true, label: true, supplierId: true },
  });
  if (locationHit) {
    const err = new Error(DUPLICATE_GSTIN_MESSAGE);
    err.statusCode = 409;
...
async function validateRegisteredGstin(gstin, stateId) {
  ...
  if (!normalized) {
    if (stateId != null) {
      ...
      return { gst: null, stateId: manualState.id, state: manualState.stateName, stateCode: manualState.stateCode };
    }
    return { gst: null, stateId: null, state: null, stateCode: null };
  }
  ...
  return {
    ...
    stateCode: resolvedState?.stateCode ?? null,
  };
}

function normalizeLocationInput(raw, index) {
  const label = String(raw.label ?? "").trim();
  if (!label) {
    const err = new Error(`Supply location ${index + 1}: label is required.`);
    err.statusCode = 400;
    throw err;
  }
...
async function validateSupplierLocations(locations, supplierIdForExclude) {
  const normalized = (locations ?? []).map(normalizeLocationInput);
  ...
  for (let i = 0; i < normalized.length; i += 1) {
    ...
    if (row.gst) {
      ...
      if (formatMsg) {
        const err = new Error(`Supply location "${row.label}": ${formatMsg}`);
        err.statusCode = 400;
        throw err;
      }
      ...
      if (stateErr) {
        const err = new Error(`Supply location "${row.label}": ${stateErr}`);
        err.statusCode = 400;
        throw err;
      }
      row.stateId = state?.id ?? row.stateId;
      await assertGstinUnique(row.gst, {
        excludeSupplierId: supplierIdForExclude,
      ...
      if (!state) {
        const err = new Error(`Supply location "${row.label}": invalid state.`);
        err.statusCode = 400;
        throw err;
      }
    }
...
async function syncSupplierLocations(tx, supplierId, locations) {
  const existing = await tx.supplierLocation.findMany({
    where: { supplierId },
    select: { id: true },
  });
  const keepIds = new Set(locations.map((a) => a.id).filter(Boolean));
  const deleteIds = existing.map((e) => e.id).filter((id) => !keepIds.has(id));
  if (deleteIds.length) {
    await tx.supplierLocation.deleteMany({ where: { id: { in: deleteIds } } });
  }
  ...
  for (const row of locations) {
    ...
    if (row.id) {
      await tx.supplierLocation.update({ where: { id: row.id }, data });
    } else {
      await tx.supplierLocation.create({
